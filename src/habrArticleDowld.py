...
class HabrArticleDowld():
    ...
    def define_numer_of_pages(self, url, type_articles, count):
        r = requests.get(url)
        url_soup = BeautifulSoup(r.text, 'lxml')
        ...
        for span in spans:
            print(span)
            l_span.append(re.sub('\s+', ' ', span.text.strip().replace('\n', '')))

        # span_idx = [i for i, item in enumerate(l_span) if item.startswith('Закладки')].pop()
        # span_idx = [i for i, item in enumerate(l_span) if re.search('^Закладки', item)]

        if type_articles == 'u':
            span_idx = [i for i, item in enumerate(l_span) if item.startswith('Профиль')].pop()
            span = spans[span_idx]
    ...
    def main(self, url, dir, type_articles, count_posts):
        ...
        self.get_articles(url, type_articles, count_posts)
